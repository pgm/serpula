...
- `import foo`, `import foo.bar`, `import foo.bar as x` — compiled to `__import__()` calls identical to CPython; relative imports (`from . import foo`) and `from foo import *` are not supported
- `from foo import bar`, `from foo import bar as x, baz` — likewise
...
### Not supported
`nonlocal`, `try`/`except`/`finally`, `with`, `yield`/generators, decorators, `async`/`await`, walrus operator (`:=`), `match`/`case`, chained comparisons (`1 < x < 10`), keyword-only params (`def f(*, x)`), lambda.
