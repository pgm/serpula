...
from vm import execute, resume

def execute_from_source(source):
    exe = compile(source)
    return execute(exe, {})

def test_suspend_returns_args_as_tuple():
    rt = execute_from_source('suspend("a", "b", "c")')
    assert rt.suspended
